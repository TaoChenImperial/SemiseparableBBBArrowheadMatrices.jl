...
This intermediate factors matrix {\it does not} have BPS structure, however, we shall so that it is a structured perturbation of BPS.


\section{Structure Preservation of the QR Factors for BPS Matrices}
\label{sec:main}
...
\begin{definition}
We denote the set of {\it banded-plus-semseparable matrices} (BPS) with lower-semisepable rank $r$, upper-semiseparable rank $p$, lower-bandwidth $\ell$ and upper-bandwidth $m$  by $\BPS \subset \bbR^{n \times n}$, where $A \in \BPS \subset \bbR^{n \times n}$ if
\[
