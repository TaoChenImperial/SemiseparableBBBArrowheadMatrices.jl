...
F = \begin{bmatrix}
r_{11} & r_{12} & r_{13} & \cdots & r_{1n} \\
y_{21} & r_{22} & r_{23} & \cdots & r_{2n} \\
y_{31} & y_{32} & r_{33} & \cdots & r_{3n} \\
\vdots & \vdots & \vdots & \ddots & \vdots \\
y_{n1} & y_{n2} & y_{n3} & \cdots & r_{nn}
\end{bmatrix}
\end{equation*}
...
\end{bNiceMatrix},
\]
alongside $\bftau_k = [\tau_1,\ldots,\tau_k,0,\ldots,0]^\top$. We can view applying the $k$-th Householder transformation as a map
\[
...
\section{Structure Preservation of the factors matrix for BPS matrices}
\label{sec:main}
...
We will also need to work with specific columns of banded matrices, which are convenient to express in terms of an explicit sparsity structure:
...
We will now describe the structure of each portion of the partial factors matrix.
The first $k$-th rows/columns of $F_k$ will maintain the BPS structure albeit with an increased upper semiseparable rank of $p+r$.
The bottom right will be a structured perturbation of a BPS matrix. In particular,
the partial factors matrices will satisfy the following: for $B_k \in {\mathrm{BM}}_{(\ell,\ell+m)}^{n \times n}$,
$V_k \in \bbR^{n \times r}$, $W_k \in \bbR^{n \times (r+p)}$, and
\[
\tilde S :=  \begin{bmatrix} S & A^\top U
\end{bmatrix} \in \bbR^{n \times (r+p)},
\]
we have the first $k$ rows/columns satisfying the conditions of the BPS format:
\begin{align*}
...
\end{align*}
where $P_k \in \calP(A[k+1\!:\!n,k+1\!:\!n])$ for the following perturbation space:
...
We claim this structure is preserved: if $F_k$ has this form then so does $F_{k+1}$. The basis case $F_0 := A$ is trivial: we can take the perturbation to be zero.

As a preliminary step we note this structure is preserved under trunctation:
...
\end{align*}
The theorem follows since we have
\begin{align*}
...
The first step is to understand the structure of the Householder reflector, which corresponds to rows $k+1$ through $n$ of
the $k$-th column of $F_k$:

\begin{lemma}
    Suppose $F_{k-1}$ has the prescribed form. Then there exists $B_k$ and $V_k$ such that
\[
...
\begin{lemma}
    Suppose $F_{k-1}$ has the prescribed form. Then there exists $B_k, V_k, W_k$ such that
\[
F_k[1:k,:] = (B_k + \tril(U V_k^\top, -1) + \triu(W_k \tilde S^\top,1))[1:k,:]
\]
\end{lemma}
...
&= A[k\!:\!n,k\!:\!n] +
\underbrace{(U[k\!:\!n,:]  J+ X[k\!:\!n,:])}_{=:T_1 \in \bbR^{(n-k+1) \times r}} S[k\!:\!n,:]^\top  \\
...
\[
\bfe_1^\top A[k\!:\!n,k\!:\!n] = 
B[k,k\!:\!n]^\top + \underbrace{\bfe_k^\top \triu(W S^\top,1)}_{ 
    W[k,:]^\top S[k:n,:]^\top   - W[k,:]^\top S[k,:] \bfe_1^\top
}
\]
we can write the first row as
\begin{align*}
\bfe_1^\top A_{k-1} &= \bfs_1^\top  + \underbrace{(W[k,:]^\top + \bfe_1^\top T_1)}_{=:\bfw_1^\top \in \bbR^{1 \times r}} S[k\!:\!n,:]^\top  + \underbrace{\bfe_1^\top T_2}_{=:\tilde\bfw_1^\top \in \bbR^{1 \times r}} (A^\top U)[k\!:\!n,:]^\top.
\end{align*}
...
\underbrace{\bfs_2^\top +  T_3  \tilde\bfy_k^\top}_{=: \bfs_3^\top \in \calS_{1 \times \ell+m+1}^{1 \times (n-k+1)}} + \underbrace{(V_k[k,:]^\top  + \tilde\bfy_k^\top T_2)}_{=: \tilde \bfw_2^\top \in \bbR^{(n-k+1) \times r}} (A^\top U)[k\!:\!n,:]^\top\\
&\qquad + 
  \underbrace{(\bfb_k^\top W[k\!:\!n,:] + \tilde\bfy_k^\top T_1)}_{=: \bfw_2^\top \in \bbR^{(n-k+1) \times r}}  S[k\!:\!n,:]^\top.
\end{align*}
...
\begin{lemma}
    Suppose $F_{k-1}$ has the prescribed form. Then so does $F_k$. 
\end{lemma}

\begin{proof}
    We have already showed the result apart from the bottom right
\[
A_k := F_k[(k+1)\!:\!n,(k+1)\!:\!n] = 
 ((I - \tau_k \tilde \bfy_k \tilde \bfy_k^\top)A_{k-1})[(k+1)\!:\!n,(k+1)\!:\!n].
\]
...
We begin by establishing notation for matrix subblocks. For a matrix $M \in \mathbb{R}^{n \times n}$, let $M[i:j, m:n]$ denote the submatrix consisting of rows $i$ through $j$ and columns $m$ through $n$ (inclusive). When extracting a single row or column, the result is a column vector:
...
Define vectors \( \bar{\mathbf{u}}_k = U[k,:] \in \mathbb{R}^r \), \( \bar{\mathbf{v}}_k = V[k,:] \in \mathbb{R}^r \), \( \bar{\mathbf{w}}_k = W[k,:] \in \mathbb{R}^p \), and \( \bar{\mathbf{s}}_k = S[k,:] \in \mathbb{R}^p \) for \( k=1,\ldots,n \). The matrix \( A \) can then be expressed element-wise as:
\begin{equation*}
A =
\begin{bmatrix}
...
Next, we introduce two key definitions and a pivotal lemma.
...
\begin{align*}
\calV_j(A) &:= \bigg\{\mathbf{d} +  S[j:n,:]\boldsymbol{\alpha} + (A^\top U)[j:n,:]\boldsymbol{\beta} : \\
&\qquad\quad \mathbf{d} = \begin{bmatrix} \mathbf{d}_s \in \mathbb{R}^{\min(\ell+m,n+1-j)}  \\ \mathbf{0} \end{bmatrix} \in \mathbb{R}^{n+1-j}, \boldsymbol{\alpha} \in \mathbb{R}^p , \boldsymbol{\beta} \in \mathbb{R}^r\bigg\} \subset \bbR^{n+1-j}.
\end{align*}
\end{definition}


\begin{lemma}\label{lemma:structure_preserve}
Given \( A \in \BPS \) and  \( P \in \calP(A) \), let \( \tilde{C} = (I - \tau \bfy \bfy^\top) (A+P) \) where $\tau$ is a coefficient, $\bfy = \mathbf{e}_1 + U^{(2)}\bar{\mathbf{k}} + \mathbf{b}$ with $\mathbf{e}_1$ being the first standard basis vector, $U^{(2)}\in\mathbb{R}^{n\times r}$ satisfying $U^{(2)}[1,:]=\mathbf{0}$ and $U^{(2)}[2:n,:]=U[2:n,:]$, $\bar{\mathbf{k}}\in\mathbb{R}^r$, and $\mathbf{b}\in\mathbb{R}^{n}$ nonzero only in entries $2$ through $\min(\ell+1, n)$. Then the following hold:
\begin{enumerate}
...
Next define the auxiliary vectors: \(\mathbf{c}_1 = J^\top U^\top \bfy \in \mathbb{R}^p\), \(\mathbf{c}_2 = K^\top U^\top \bfy \in \mathbb{R}^r\), \(\mathbf{c}_3 = U^\top \bfy \in \mathbb{R}^r\), \(\mathbf{c}_4 = X^\top \bfy \in \mathbb{R}^p\), \(\mathbf{c}_5 = Y^\top \bfy \in \mathbb{R}^r\), \(\mathbf{c}_6 = Z^\top \bfy \in \mathbb{R}^{n}, \quad \text{which has the form } \mathbf{c}_6 = \begin{bmatrix} \mathbf{c}_{6s} \\ \mathbf{0} \end{bmatrix} \text{ with } \mathbf{c}_{6s} \in \mathbb{R}^{\min(\ell+m,n)}\).

Also, let \( \mathbf{x}^{(1)} = X[1,:] \in \mathbb{R}^p \), \( \bfy^{(1)} = Y[1,:] \in \mathbb{R}^r \), and \( \mathbf{z}^{(1)} = Z[1,:] \in \mathbb{R}^n \).

We now compute \( (I - \tau \bfy \bfy^\top) C \) by distributing the transformation over each term in the definition of \( C \).To maintain clarity in the following derivations, we use the label "nonzero entries" to specifically denote dense subblocks or vectors whose support is confined to the leading indices (determined by $\ell$ and $m$). These terms are systematically tracked to ensure they map directly onto the structured generators $\{J, K, E, X, Y, Z\}$ defined in the space $\calP(A)$.

(i) Transformation of \( A \):
Substituting the expressions \( \bfy=\mathbf{e}_1+U^{(2)}\bar{\mathbf{k}}+\mathbf{b}\), \( \mathbf{e}_1^\top A=\mathbf{d}_1^\top+\bar{\mathbf{w}}_1^\top S^\top \), and \( \mathbf{b}^\top A=\bar{\mathbf{d}}^\top+\mathbf{f}^\top S^\top \), we obtain:
\begin{equation}\label{proof_first}
\begin{aligned}
(I - \tau \bfy \bfy^\top)A = & A + \mathbf{e}_1 \big[ \underbrace{(-\tau \mathbf{d}_1^\top - \tau \bar{\mathbf{d}}^\top)}_{\min(\ell+m+1,n)\hbox{ nonzero entries}} + \underbrace{(-\tau \bar{\mathbf{w}}_1^\top - \tau \mathbf{f}^\top)}_{\in \bbR^{1 \times p}}S^\top \\
...
(ii) Transformation of \( UJS^\top \): Substituting the expressions \( \bfy^\top UJ=\mathbf{c}_1^\top \), \( \bfy=\mathbf{e}_1+U^{(2)}\bar{\mathbf{k}}+\mathbf{b}\), and \( U=\mathbf{e}_1\bar{\mathbf{u}}_1^\top+U^{(2)}\), we obtain
\begin{equation}
\begin{aligned}
(I - \tau \bfy \bfy^\top)UJS^\top = & \mathbf{e}_1\underbrace{(\bar{\mathbf{u}}_1^\top J - \tau \mathbf{c}_1^\top)}_{\in \bbR^{1 \times p}}S^\top + U^{(2)}\underbrace{(J - \tau \bar{\mathbf{k}} \mathbf{c}_1^\top)}_{\in \bbR^{r \times p}}S^\top + \underbrace{(-\tau \mathbf{b} \mathbf{c}_1^\top)}_{\bbR^{\min(\ell+1,n)\times p} \hbox{ nonzero entries}}S^\top.
\end{aligned}
\end{equation}

(iii) Transformation of \( UKU^\top A \): Substituting the expressions \( \bfy^\top UK=\mathbf{c}_2^\top\), \( \bfy=\mathbf{e}_1+U^{(2)}\bar{\mathbf{k}}+\mathbf{b}\), \( U=\mathbf{e}_1\bar{\mathbf{u}}_1^\top+U^{(2)}\), and \( \mathbf{e}_1^\top A=\mathbf{d}_1^\top+\bar{\mathbf{w}}_1^\top S^\top\), we obtain
\begin{equation}
\begin{aligned}
& (I - \tau \bfy \bfy^\top)UKU^\top A \\
...
(iv) Transformation of \( UE \): Substituting the expressions \( \bfy^\top U=\mathbf{c}_3^\top \), \( \bfy=\mathbf{e}_1+U^{(2)}\bar{\mathbf{k}}+\mathbf{b}\), and \( U=\mathbf{e}_1\bar{\mathbf{u}}_1^\top+U^{(2)}\), we obtain
\begin{equation}
\begin{aligned}
(I - \tau \bfy \bfy^\top)UE = & \mathbf{e}_1\underbrace{(\bar{\mathbf{u}}_1^\top E - \tau \mathbf{c}_3^\top E)}_{\min(\ell+m+1,n)\hbox{ nonzero entries}
...
(v) Transformation of \( XS^\top \): Substituting the expressions \(\bfy^\top X=\mathbf{c}_4^\top \) and \(\bfy=\mathbf{e}_1+U^{(2)}\bar{\mathbf{k}}+\mathbf{b}\), we obtain
\begin{equation}
\begin{aligned}
(I - \tau \bfy \bfy^\top)XS^\top = \mathbf{e}_1\underbrace{(-\tau \mathbf{c}_4^\top)}_{\in \bbR^{1 \times p}}S^\top + U^{(2)}\underbrace{(-\tau \bar{\mathbf{k}} \mathbf{c}_4^\top)}_{\in \bbR^{r \times p}}S^\top + \underbrace{(X - \tau \mathbf{b} \mathbf{c}_4^\top)}_{\bbR^{\min(\ell+1,n)\times p} \hbox{ nonzero entries}}S^\top.
\end{aligned}
\end{equation}

(vi) Transformation of \( YU^\top A \): Substituting the expressions \(\bfy^\top Y=\mathbf{c}_5^\top \), \(\bfy=\mathbf{e}_1+U^{(2)}\bar{\mathbf{k}}+\mathbf{b}\), \(U=\mathbf{e}_1\bar{\mathbf{u}}_1^\top+U^{(2)}\), and \(\mathbf{e}_1^\top A=\mathbf{d}_1^\top+\bar{\mathbf{w}}_1^\top S^\top \), we obtain
\begin{equation}
\begin{aligned}
& (I - \tau \bfy \bfy^\top)YU^\top A \\
...
\textbf{(vii) Transformation of \( Z \):} Substituting the expressions \(\bfy^\top Z=\mathbf{c}_6^\top \) and \(\bfy=\mathbf{e}_1+U^{(2)}\bar{\mathbf{k}}+\mathbf{b}\), we obtain

\begin{equation}\label{proof_last}
\begin{aligned}
(I - \tau \bfy \bfy^\top)Z = &\mathbf{e}_1\underbrace{(-\tau \mathbf{c}_6^\top)}_{\min(\ell+m+1,n)\hbox{ nonzero entries}
...
\tilde{C}[2:n, 2:n] = \tilde{A} + \tilde{U} \tilde{J} \tilde{S}^\top + \tilde{U} \tilde{K} \tilde{U}^\top \tilde{A} + \tilde{U} \tilde{E} + \tilde{X} \tilde{S}^\top + \tilde{Y} \tilde{U}^\top \tilde{A} + \tilde{Z},
\end{equation}
where
\begin{align*}
    \tilde{A} &= A[2:n, 2:n] \\
...
Secondly, the first row of the transformed matrix, \( \tilde{C}[1, 2:n] \), can be expressed as:
\begin{equation*}
\tilde{C}[1, 2:n] = \hat{\mathbf{d}} +  S[2:n, :]\boldsymbol{\alpha} + (A^\top U^{(2)})[2:n, :]\boldsymbol{\beta},
\end{equation*}
where
\begin{align*}
    \hat{\mathbf{d}} &= \begin{bmatrix} \hat{\mathbf{d}}_s \\ \mathbf{0} \end{bmatrix} \in \mathbb{R}^{n-1}, \quad \text{with} \\
    \hat{\mathbf{d}}_s &= (\mathbf{d}_1 - \tau \mathbf{d}_1 - \tau \bar{\mathbf{d}} + \mathbf{d}_1 \bar{\mathbf{u}}_1^\top K^\top \bar{\mathbf{u}}_1   - \tau \mathbf{d}_1 \bar{\mathbf{u}}_1^\top \mathbf{c}_2 + E^\top \bar{\mathbf{u}} - \tau E^\top \mathbf{c}_3 \\
    &\quad + \mathbf{d}_1 \bar{\mathbf{u}}_1^\top \bfy^{(1)} - \tau \mathbf{d}_1 \bar{\mathbf{u}}_1^\top \mathbf{c}_5 + \mathbf{z}^{(1)} - \tau \mathbf{c}_6)[2:\min(\ell+m+1, n)], \\
    \hat{\boldsymbol{\alpha}} &= (\bar{\mathbf{w}}_1 - \tau \bar{\mathbf{w}}_1 - \tau \mathbf{f} + J^\top \bar{\mathbf{u}}_1 - \tau \mathbf{c}_1 + \bar{\mathbf{w}}_1 \bar{\mathbf{u}}_1^\top K^\top \bar{\mathbf{u}}_1  - \tau \bar{\mathbf{w}}_1\bar{\mathbf{u}}_1^\top \mathbf{c}_2 \\
    &\quad + \mathbf{x}^{(1)} - \tau \mathbf{c}_4 + \bar{\mathbf{w}}_1 \bar{\mathbf{u}}_1^\top \bfy^{(1)}  - \tau \bar{\mathbf{w}}_1 \bar{\mathbf{u}}_1^\top \mathbf{c}_5) \in \mathbb{R}^p, \\
    \hat{\boldsymbol{\beta}} &= (-\tau \bar{\mathbf{k}} + K^\top \bar{\mathbf{u}}_1 - \tau \mathbf{c}_2 + \bfy^{(1)} - \tau \mathbf{c}_5) \in \mathbb{R}^r.
\end{align*}
...
\tilde{C}[1, 2:n] = \tilde{\mathbf{d}} +  S[2:n, :]\tilde{\boldsymbol{\alpha}} + (A^\top U)[2:n, :]\tilde{\boldsymbol{\beta}},
\end{equation}
where
\begin{align*}
    \tilde{\mathbf{d}} &= \begin{bmatrix} \tilde{\mathbf{d}}_s \\ \mathbf{0} \end{bmatrix} \in \mathbb{R}^{n-1}, \quad \text{with} \\
    \tilde{\mathbf{d}}_s &= (\mathbf{d}_1 - \tau \mathbf{d}_1 - \tau \bar{\mathbf{d}} + E^\top \bar{\mathbf{u}}_1 - \tau E^\top \mathbf{c}_3 + \mathbf{z}^{(1)} - \tau \mathbf{c}_6 + \tau \mathbf{d}_1\bar{\mathbf{u}}_1^\top \bar{\mathbf{k}})[2:\min(\ell+m+1, n)], \\
    \tilde{\boldsymbol{\alpha}} &= (\bar{\mathbf{w}}_1 - \tau \bar{\mathbf{w}}_1 - \tau \mathbf{f} + J^\top \bar{\mathbf{u}}_1 - \tau \mathbf{c}_1 + \mathbf{x}^{(1)} - \tau \mathbf{c}_4 + \tau \bar{\mathbf{w}}_1 \bar{\mathbf{u}}_1^\top \bar{\mathbf{k}}) \in \mathbb{R}^p, \\
    \tilde{\boldsymbol{\beta}} &= (-\tau \bar{\mathbf{k}} + K^\top \bar{\mathbf{u}}_1 - \tau \mathbf{c}_2 + \bfy^{(1)} - \tau \mathbf{c}_5) \in \mathbb{R}^r.
\end{align*}

This confirms that \( \tilde{C}[1, 2:n] \in \calV_2(A) \), completing the proof of the lemma.
\end{proof}

\subsection{Structure Preservation Theorem and its Proof}
...
Let \( A^{(i)} \) denote the matrix after the \( i \)-th Householder transformation, with \( A^{(0)} = A \). Define \( A_i = A[i:n, i:n] \), \( U_i = U[i:n, :] \), \( V_i = V[i:n, :] \), \( S_i = S[i:n, :] \), and \( W_i = W[i:n, :] \). Let \( \bar{\mathbf{u}}_i = (u_i^{(1)}, \ldots, u_i^{(r)})^\top \) and \( \bar{\mathbf{w}}_i = (w_i^{(1)}, \ldots, w_i^{(p)})^\top \). Let \( \bar{S} = U^\top A \in \mathbb{R}^{r \times n} \).
...
\begin{equation*}
\begin{aligned}
A^{(j)}[j+1:n, j+1:n] = & A_{j+1} + U_{j+1} J_{j+1} S_{j+1}^\top + U_{j+1} K_{j+1} U_{j+1}^\top A_{j+1} \\
& + U_{j+1} E_{j+1} + X_{j+1} S_{j+1}^\top + Y_{j+1} U_{j+1}^\top A_{j+1} + Z_{j+1}, \label{After_HT}
\end{aligned}
\end{equation*}
...
If \( j < n-1 \), we now perform the \( (j+1) \)-th Householder transformation on $A_{j+1}+P_{j+1}$. Let \( \bfy_{j+1} \) be the corresponding Householder vector. It can be expressed as  \(\bfy_{j+1}[1:j] = \mathbf{0}\) and \( \bfy_{j+1}[j+1:n] = \mathbf{e}_{j+1} + U^{(j+2)} \bar{\mathbf{k}}_{j+2} + \mathbf{b}_{j+2} \), where \( \mathbf{e}_{j+1} \in \mathbb{R}^{n-j} \) is the first standard basis vector, \( U^{(j+2)} \in \mathbb{R}^{(n-j) \times r} \) satisfies \( U^{(j+2)}[1,:] = \mathbf{0} \) and \( U^{(j+2)}[2:n-j, :] = U[j+2:n, :] \), \( \bar{\mathbf{k}}_{j+2} \in \mathbb{R}^r \), and \( \mathbf{b}_{j+2} \in \mathbb{R}^{n-j} \) is non-zero only in entries $2$ through \( \min(\ell+1, n-j) \). This vector defines the \( (j+1) \)-th column of \( F \):
\begin{equation}
F[j+2:n, j+1] = (U \bar{\mathbf{k}}_{j+2})[j+2:n] + \mathbf{b}_{j+2}[2:n-j]. \label{F_tril_2}
\end{equation}

We now apply Lemma \ref{lemma:structure_preserve} to \( A^{(j)}[j+1:n, j+1:n] \), which is \( A_{j+1} + P_{j+1}\). The Householder transformation \( (I - \tau_{j+1} \bfy_{j+1}[j+1:n] \bfy_{j+1}[j+1:n]^\top) \) is applied to \( A_{j+1}+P_{j+1} \), here \(\tau_{j+1}\) is a coefficient found to satisfy the definition of a Householder transformation.
...
\begin{equation}
\begin{aligned}
F[j+1, j+2:n] = & \tilde{\mathbf{d}}_{j+2} + (S\tilde{\boldsymbol{\alpha}}_{j+2})[j+2:n] + (\bar{S}^\top\tilde{\boldsymbol{\beta}}_{j+2})[j+2:n], \label{F_triu_2}
\end{aligned}
\end{equation}
