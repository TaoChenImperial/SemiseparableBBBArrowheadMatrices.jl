...
In this paper, we close this theoretical gap. We prove that the QR factorization of a BPS matrix yields a factor matrix \( F \), which is the matrix whose upper triangular part contains $R$ and lower triangular part contains the Householder reflectors encoding $Q$,  that is itself BPS. The main idea is that whilst BPS structure in the factor matrix is not preserved during the intermediate stages of the QR factorisation, the factor matrix is a structured perturbation of a BPS matrix with a precise form (See Definition~\ref{def:hmbpsm}). When the QR factorisation completes the perturbation vanishes and we are left with a BPS matrix. This understanding of the structure enables the design of an \( O(n) \) QR factorization. Furthermore, the same idea of understanding the structure of perturbations also facilitates a complete direct solver: applying \( Q^\top \) and performing backward substitution on the structured  \( R \) are also achieved in linear time. Our work thus provides a QR-based \( O(n) \) solution for BPS systems. Furthermore, we show that for symmetric BPS matrices, the $RQ$ product (central to the QR eigenvalue algorithm) also retains the BPS structure via an intermediate structured perturbation, enabling efficient eigenvalue computations.
...
\sotodoinline{Add more detailed explanation of key ideas pointing to definitions}
...
\sotodoinline{None of the section titles are informative. We need to restructure.}
...
\sotodoinline{The following discussion on Factors matrices should be its own section}
...
\begin{definition}
Given \( A \in  \BPS \) and $j=1, 2, ..., n$, define the vector space\sotodo{Why is the following a row vector?}
\begin{align*}
...
\begin{proof}
Let us introduce some necessary notations first:\sotodo{Wasn't this notation introduced already?}
...
(iii) Transformation of \( UKU^\top A \): Substituting the expressions \( \mathbf{y}^\top UK=\mathbf{c}_2^\top\), \( \mathbf{y}=\mathbf{e}_1+U^{(2)}\bar{\mathbf{k}}+\mathbf{b}\), \( U=\mathbf{e}_1\bar{\mathbf{u}}_1^\top+U^{(2)}\), and \( \mathbf{e}_1^\top A=\mathbf{d}_1^\top+\bar{\mathbf{w}}_1^\top S^\top\), we obtain\sotodo{What does "nonzero entries" mean?}
\begin{equation}
\begin{aligned}
...
\end{equation}
we can also express this row in the form:\sotodo{Is the transpose here natural?}
\begin{equation}
\begin{aligned}
...
\For{$k = 1$ to $n-1$}
    ...
    \State \textbf{Step 4: Update HMBPSM matrices \sotodoinline{Remove references to HMBPSM}}
...
\begin{algorithm}
\caption{FormHouseholderVector \sotodoinline{Just refer to algorithm numbers and give names} (Step 1 of Algorithm \ref{algo:fastqr})}
...
\begin{algorithm}
\caption{ComputeRowUpdate  \sotodoinline{Just refer to algorithm numbers and give names}  (Step 3 of Algorithm \ref{algo:fastqr})}
...
\begin{algorithm}
\caption{UpdateHMBPSM  \sotodoinline{Just refer to algorithm numbers and give names}  (Step 4 of Algorithm \ref{algo:fastqr})}
...
This structure implies that each Householder vector \( \mathbf{y}_k \) can be expressed as:\sotodo{Why the bar?}
\begin{equation}
...
The key insight is that the off-diagonal entries \( R[j,k] \) for \( k > j \) can be decomposed as\sotodo{Why the cdot?}:
\[
...
Given an $n\times n$ upper triangular matrix $R$ and an $n\times r$ matrix $U$, define the vector space:\sotodoinline{The following is not a vector space}
\begin{align*}
\calH(R;U) &:= \bigg\{
...
\sotodoinline{Make well-conditioned}}
\label{fig:stability}
\end{figure}
