...
Intermediate stages of the Householder reflection correspond to a partial factors matrix. In particular, if we apply $k$ Householder reflections to partially upper triangularize $A$ as
\[
(I - \tau_k \bfy_k \bfy_k^\top) \cdots (I - \tau_1 \bfy_1 \bfy_1^\top) A =
\begin{pNiceMatrix}[cell-space-limits=5pt]
...
We can represent this partial factorisation using the intermediate factor matrix
\[
F_k = \begin{pNiceMatrix}[cell-space-limits=10pt,columns-width=20pt]
r_{11}      & r_{12} & \cdots       & r_{1k} & r_{1,k+1} & \cdots & r_{1n} \\
...
\CodeAfter
  \tikz \draw ([xshift=0pt,yshift=2pt]1.5-|1) -- ([xshift=8pt,yshift=0pt]5-|4.5);
    \tikz \draw ([xshift=8pt,yshift=0pt]5-|4.5) -- ([xshift=8pt,yshift=0pt]5-|8.5);
...
\begin{definition}
Denote the set of {\it banded-plus-semseparable matrices} (BPS) with lower-semisepable rank $r$, upper-semiseparable rank $p$, lower-bandwidth $\ell$, and upper-band-width $m$  as $\BPS \subset \bbR^{n \times n}$. In particular, $A \in \BPS \subset \bbR^{n \times n}$ if
\[
A = B + \tril(UV^\top, -1) + \triu(WS^\top, 1)\label{express_A}
\]
for \( U, V \in \mathbb{R}^{n\times r} \), \( W, S \in \mathbb{R}^{n\times p} \), and \( B = (b_{kj})_{k,j=1}^n \in \mathbb{R}^{n \times n} \) is a banded matrix satisfying \( b_{kj}=0 \) for \( k-j>l \) or \( j-k>m \).
\end{definition}

We note that the BPS structure is not uniquely defined, so we assume if we know $A \in BPS$ we have access to $U,V,W,S$. Note this structure is preserved under principle subsection:
\begin{align*}
A[k\!:\!n, k\!:\!n] &= B[k\!:\!n, k\!:\!n] 
+ 
...
The first $k$-th rows/columns of $F_k$ will maintain the BPS structure, however, the bottom right will be a structured perturbation of a BPS matrix. In particular, 
the factors matrices will be of the form
$$
F_k = A_k + \begin{bmatrix} \bfzero & \bfzero\\
    \bfzero & P_k
\end{bmatrix}
$$
where $A_k \in \BPS$ and $P_k \in \calP(A_k[k\!+\!1\!:\!n,k\!+\!1\!:\!n])$ for the following perturbation space:
...
The first step is to understand the structure of the Householder reflector, which corresponds to the $k$-th column of $F_k$:

\begin{lemma}
    Suppose $F_{k-1}$ has the prescribed form. Then the first $k$ columns of $F_k$ also have the prescribed form.
\end{lemma}
\begin{proof}
The $(k+1)$-th Householder reflection is defined as
\[
\bfy_k = 
\]
...
\begin{lemma}
    Suppose $F_k$ has the prescribed form. Then the first $(k+1)$-th columns of $F_{k+1}$ also have the pescribed form.
\end{lemma}
