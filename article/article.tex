...
\begin{abstract}
We show that each stage of the QR factorization of banded-plus-semiseparable matrices computed using Householder reflections has a specific structured perturbation. This theoretical result enables the design of linear-complexity algorithms for QR factorization and for solving the associated linear systems. Numerical experiments validate the optimal linear complexity  and demonstrate substantial speedups compared with existing hierarchical approaches. The algorithms have been implemented in an open-source Julia package, providing an efficient and accessible platform for practical use.
\end{abstract}
...
\sotodoinline{Do we want to allow for complex numbers?}

\sotodoinline{What to say about QR algorithm?}

\sotodoinline{Add a nullspace problem coming from the ultraspherical spectral method?}

\sotodoinline{Does it generalise to rectangular?}

\sotodoinline{Say something about inverses of banded matrices being banded-plus-semiseparable?}

\sotodoinline{Add stability plot}

Banded-plus-semiseparable (BPS) matrices, expressible as \sotodo{Change all the ${}^T$ to ${}^\top$}
\begin{equation*}
A = \underbrace{B}_{\text{banded}} + \underbrace{\tril(UV^T, -1)}_{\text{lower semiseparable, rank } r} + \underbrace{\triu(WS^T, 1)}_{\text{upper semiseparable, rank } p} \in \bbR^{n \times n},
\end{equation*}
 arise in numerous applications from PDEs with non-local interactions~\cite{knook2024quasi}\sotodo{This is a bizarre reference and description. Is it AI generated?} to signal processing, control theory, and eigenvalue problems~\cite{vandebril2005note}\sotodo{Add citations to Arieh Iserles W-systems papers}. Their structure requires only $O(n)$ storage which invites the development of \( O(n) \) algorithms, a goal successfully achieved for iterations of the QR algorithm for symmetric semiseparable systems~\cite{vandebril2005implicit}, and for solving linear systems with \emph{diagonal}-plus-semiseparable matrices~\cite{eidelman1997inversion}. 
...
Define vectors \( \bar{\mathbf{u}}_i = U[i,:]^T \in \mathbb{R}^r \), \( \bar{\mathbf{v}}_i = V[i,:]^T \in \mathbb{R}^r \)\sotodo{Shouldn't these be the columns of $V$? and why the bar?}, \( \bar{\mathbf{w}}_i = W[i,:]^T \in \mathbb{R}^p \), and \( \bar{\mathbf{s}}_i = S[i,:]^T \in \mathbb{R}^p \) for \( i=1,\ldots,n \). The matrix \( A \) can then be expressed element-wise as: \sotodo{Only number equations that are cited}
\begin{equation}
A =
\begin{bmatrix}
...
Before proceeding with the detailed proof, let us clarify the precise structure of the factor matrix \( F \) obtained from the QR factorization. In this work, following the convention of LAPack \sotodo{add citation}, we employ a compact representation that stores the complete information of the QR factorization in a single matrix:
...
While the final factor matrix of a QR factorization is a BPS matrix, at  intermediate  stages it has a specific structured perturbation. Here we describe this structure in terms of a linear space that, at each stage, the principle submatrix lies in:

\begin{definition}[BPS Factors Perturbation Matrix]\label{def:hmbpsm}
Given 
\[
A = B + \tril(UV^T, -1) + \triu(WS^T, 1)  \in \BPS 
\]
a \emph{BPS Factors Perturbation Matrix} (BPSFPM) is in the vector space 
\begin{align*}
\calP(A) &:= \bigg\{
...
\begin{definition}[BPS Factors Vector]
Given \( A \in  \BPS \) a \emph{BPS Factors Vector} (BPSFV) is a row vector\sotodo{should we change to column vector?} in the vector space
\begin{align*}
...
\sotodoinline{Make an algorithm environment. Possibly split into sub-algorithms}

\begin{my_algorithm}[Fast QR]\label{algo:fastqr}

This algorithm computes the QR factorization of a BPS matrix $A = B + \tril(UV^T, -1) + \triu(WS^T, 1)$, producing the structured factor matrix $F$ (which contains both $R$ and the Householder vectors) and the scalar coefficients $\boldsymbol{\tau}$, in $O(n)$ operations.
...
4. \textbf{Output:}
\begin{itemize}
    \item The complete QR factorization is represented by the structured factor matrix $F$, defined as $B_F + \tril(U \bar{K}^T, -1) + \triu([\bar{A},\bar{B}] [S,\bar{S}^T]^T, 1)$, and the vector $\boldsymbol{\tau}$.
\end{itemize}
...
At step \( j \), the\sotodo{Inner product? I prefer ${}^\top$ to $\cdot$} product \( W_F[j,:] \cdot \mathbf{s} \) thus captures all semiseparable contributions from previously computed solution components. After computing \( x_j \), the accumulator is updated to include its contribution.
...
\sotodoinline{Fix capitalization}
