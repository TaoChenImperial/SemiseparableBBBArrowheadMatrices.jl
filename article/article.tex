...
A_k :&= F_k[(k\!+\!1)\!:\!n,(k\!+\!1)\!:\!n] \\
&= 
 ((I - \tau_k \tilde \bfy_k \tilde \bfy_k^\top)A_{k-1})[2\!:\!n-k,2\!:\!n-k] \in \calP(A[(k+1)\!:\!n,(k+1)\!:\!n]).
\end{align*}
From Lemma~\ref{Lemma:Ppertub} we know that since $A_{k-1} \in \calP(A[k\!:\!n,k\!:\!n])$ we have
\[
A_{k-1}[2\!:\!(n-k),2\!:\!(n-k)] \in \calP(A[(k\!+\!1)\!:\!n,(k\!+\!1)\!:\!n]).
\]
...
(\bfs_3^\top + \tilde \bfw_2^\top (U^\top A)[:,k:n] +
 ...
 \underbrace{\bfb_k \bfw_2^\top}_{\in \calS_{(\ell+1) \times r}^{(n-k+1) \times p}} S[k:n,:]^\top   \\
...
\begin{proof}
    Follows immediately from the above as
\[
